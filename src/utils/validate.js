...
export function validateSchoolProfile(form) {
  ...
  if (form.sscBatchesCompletedCount === "" || form.sscBatchesCompletedCount === null || form.sscBatchesCompletedCount === undefined)
    errors.sscBatchesCompletedCount = "Total Number of SSC Batches Completed is required.";
  else if (isNaN(Number(form.sscBatchesCompletedCount)) || Number(form.sscBatchesCompletedCount) < 0)
  ...
  if (form.toiletsPerFloorCount === "" || form.toiletsPerFloorCount === null || form.toiletsPerFloorCount === undefined)
...
export function validateSchoolIntake(intake) {
  ...
  const fields = [
    ...
    { key: "namankit_boys_residential",    label: "Total Count of Boys under Namankit (Residential)" },
    { key: "namankit_boys_nonresidential", label: "Total Count of Boys under Namankit (Non-Residential)" },
    { key: "other_boys_residential",       label: "Total Count of Boys not under Namankit (Residential)" },
    { key: "other_boys_nonresidential",    label: "Total Count of Boys not under Namankit (Non-Residential)" },
    // Rows 25–28: Residential + Non-Residential Girls
    { key: "namankit_girls_residential",    label: "Total Count of Girls under Namankit (Residential)" },
    { key: "namankit_girls_nonresidential", label: "Total Count of Girls under Namankit (Non-Residential)" },
    { key: "other_girls_residential",       label: "Total Count of Girls not under Namankit (Residential)" },
    { key: "other_girls_nonresidential",    label: "Total Count of Girls not under Namankit (Non-Residential)" },
  ];
...
export function validatePerformanceRow(row) {
  ...
  if (!row.year)
    errors.year = "School Performance Year is required.";
  ...
  if (row.studentsAppeared === "" || row.studentsAppeared === null || row.studentsAppeared === undefined)
    errors.studentsAppeared = "No of Students Appeared is required.";
  else if (isNaN(Number(row.studentsAppeared)) || Number(row.studentsAppeared) < 0)
    errors.studentsAppeared = "Must be a valid positive number.";

  // Row 40 — No of Students Passed: Mandatory + Numeric
  if (row.studentsPassed === "" || row.studentsPassed === null || row.studentsPassed === undefined)
  ...
  if (
    row.studentsAppeared !== "" && row.studentsPassed !== "" &&
    !isNaN(Number(row.studentsAppeared)) && !isNaN(Number(row.studentsPassed)) &&
