...
const QUESTIONS = [
  ...
  {
    no: "6A", label: "Q.No. 6A", title: "Total students under Namankit scheme are less than or equal to 50% of total students",
    maxMarks: 10,
    criteria: [{ label: "less than or equal to 50%", marks: 10 }, { label: "equal to 50%", marks: 0 }],
  },
  {
    no: "6B", label: "Q.No. 6B", title: "Total residential students under Namankit scheme are less than or equal to 50% of total Residential students",
    maxMarks: 10,
    criteria: [{ label: "less than or equal to 50%", marks: 10 }, { label: "equal to 50%", marks: 0 }],
  },
  {
    ...
    <div style={styles.page}>
        ...
        {QUESTIONS.map((q, idx) => (
          <div key={q.no} style={styles.qCard}>
            ...
            <div style={{ display: "flex", borderBottom: "1px solid #dee2e6" }}>
              {q.criteria.map((c, ci) => (
                <div key={ci} style={{ ...styles.criteriaHdr, borderRight: ci < q.criteria.length - 1 ? "1px solid #aed6f1" : "none" }}>
                  {c.label}
                </div>
              ))}
            </div>
            ...
            <div style={{ display: "flex", borderBottom: "1px solid #dee2e6" }}>
              {q.criteria.map((c, ci) => (
                <div key={ci} style={{ ...styles.criteriaVal, borderRight: ci < q.criteria.length - 1 ? "1px solid #dee2e6" : "none" }}>
                  {c.marks.toFixed(2)}
                </div>
              ))}
            </div>
            ...
            <div style={{ padding: "10px 14px" }}>
              <div style={{ marginBottom: 10 }}>
                ...
                <input
                  ...
                  value={questionData[idx]?.poMarks || 0}
                  onChange={e => updateQ(idx, "poMarks", parseFloat(e.target.value) || 0)}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  ...
                  <textarea
                    value={questionData[idx]?.poRemarks || ""}
                    onChange={e => updateQ(idx, "poRemarks", e.target.value)}
                ...
                <div>
                  ...
                  <textarea
                    value={questionData[idx]?.atcRemarks || ""}
                    readOnly
                    style={{ ...styles.textArea, background: "#e9ecef", color: "#888", cursor: "not-allowed" }}
                  />
                </div>
              </div>
            </div>
      ...
      <div style={styles.card}>
        <div style={{ background: "#3a7fc1", color: "#fff", padding: "10px 16px", fontSize: 14, fontWeight: 600 }}>
          PO Remarks Summary (min 100 characters length required)
        </div>
        <div style={styles.cardBody}>
          <textarea
            value={poRemarksSummary}
            onChange={e => setPoRemarksSummary(e.target.value)}
            placeholder="Enter detailed remarks (minimum 100 characters)..."
            style={{ ...styles.textArea, minHeight: 80 }}
          />
          <div style={{ fontSize: 12, color: poRemarksSummary.length < 100 ? "#e74c3c" : "#28a745", marginTop: 4 }}>
            {poRemarksSummary.length} / 100 characters minimum
          </div>
        </div>
      </div>
        ...
        <div style={{ ...styles.cardBody, display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 16 }}>
          {[
            ...
            <div key={label} style={{ background: "#f8f9fa", border: "1px solid #dee2e6", borderRadius: 4, padding: "10px 14px" }}>
              ...
              <div style={{ fontSize: 16, fontWeight: 700, color: "#1a2a5e" }}>{val}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={styles.card}>
        <div style={{ ...styles.cardBody }}>
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8, color: "#333" }}>Based on Marks Fees Structure</div>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ background: "#f8f9fa" }}>
                {["Fees", "Min Marks", "Max Marks"].map(h => (
                  <th key={h} style={{ padding: "8px 12px", textAlign: "left", border: "1px solid #dee2e6", fontWeight: 600, color: "#333" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {[
                ...
                ["₹60,000", "70.00", "79.99"],
                ["₹50,000", "60.00", "69.99"],
                ["Not Eligible", "0.00", "59.99"],
              ].map(([fees, min, max]) => (
                <tr key={fees}>
                  <td style={{ padding: "8px 12px", border: "1px solid #dee2e6" }}>{fees}</td>
                  <td style={{ padding: "8px 12px", border: "1px solid #dee2e6" }}>{min}</td>
                  <td style={{ padding: "8px 12px", border: "1px solid #dee2e6" }}>{max}</td>
                </tr>
              ))}
            </tbody>
          </table>
