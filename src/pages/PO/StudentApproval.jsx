...
export default function StudentApproval() {
  ...
  // ✅ Fix 2 — View student modal state
  ...
  // ✅ Fix 1 + Fix 3 — Updated handleSave
  const handleSave = async () => {
    ...
    // ✅ Fix 1 — Remarks mandatory for approved students
    ...
    try {
      for (const row of rows) {
        // ✅ Fix 3 — Skip already approved/rejected students
      ...
      <div style={{ padding: 18 }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 12, marginBottom: 14 }}>
          <div>
            ...
            {schoolError && (
              <div style={{ color: "#e53935", fontSize: 12, marginTop: 6 }}>{schoolError}</div>
            )}
          </div>
          <button
            onClick={handleSearch}
            style={{ background: "#28a745", color: "#fff", border: "none", padding: "8px 12px", borderRadius: 4, cursor: "pointer" }}
          >
            Search
          </button>
        </div>
            ...
            <thead style={{ background: "#fafafa" }}>
              <tr>
                {["Sr. No.","Student Unq. No.","Renewal Status","Student Name","Aadhar Number","Family Income","Class","View Student Details","Approve","Remarks","Approval Status"].map((title) => (
                  <th key={title} style={{
                    padding: "12px 16px", background: "#1a2a5e", color: "#fff",
                    fontWeight: 600, fontSize: 13, textAlign: "left",
                    borderRight: "1px solid #2d3d6e", whiteSpace: "nowrap",
                  }}>
                    {title}
                  </th>
                ))}
              ...
              {rows.length === 0 ? (
                ...
                paged.map((r, i) => (
                  <tr key={r.id || i}>
                    ...
                    {/* ✅ Fix 3 — Approve disabled after saved */}
                    <td style={tdStyle}>
                      {r.status === "approved" || r.status === "rejected" ? (
                        <span style={{
                          color: r.status === "approved" ? "#28a745" : "#dc3545",
                          fontWeight: 600,
                        }}>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setApprovals((prev) => ({ ...prev, [r.id]: e.target.checked }))
                          }
                        />
                      )}
                    </td>

                    {/* ✅ Fix 1 — Remarks with validation */}
                    <td style={tdStyle}>
                      {r.status === "approved" || r.status === "rejected" ? (
                        <span>{remarks[r.id] || r.poRemarks || "—"}</span>
                      ) : (
                        <div>
                          <input
                            style={{
        ...
        {/* Save + Export buttons — unchanged */}
        <div style={{ marginTop: 14, display: "flex", gap: 8 }}>
          <button
  onClick={handleSave}
  disabled={saving || !hasPendingStudents}  // ← disabled when no pending
  style={{
...
>
  {saving ? "Saving..." : "Save"}
          ...
          <button
            ...
            style={{ background: "#f0ad4e", color: "#fff", border: "none", padding: "8px 12px", borderRadius: 4 }}
          >
            Export to Excel
          </button>
        </div>
      ...
      {viewStudent && (
        <div style={{
          ...
          <div style={{
            ...
            minWidth: 400, maxWidth: 600, width: "90%",
                ...
                {[
                  ...
                  <tr key={label}>
                    <td style={{
                      ...
                      background: "#f8f9fa", border: "1px solid #dee2e6", width: "40%"
                    }}>
                      {label}
                    </td>
